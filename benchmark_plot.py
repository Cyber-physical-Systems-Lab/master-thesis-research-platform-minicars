...
----------------------------------------
Top-level keys
  meta              : {scenario, policy, calibration, car_ids, n_frames,
                       d_col_px, d_safe_px, d_warn_px, saved_at}
  frames            : list of frame dicts
  interaction_zones : {interactions: [{car_id, taus}]}
  track_ground_truth: {lane1_ref, lane2_ref}  -- list of [x, y] pairs
...
def frames_to_arrays(frames: List[dict], car_id: str) -> dict:
    ...
    def _safety(f):
        ...
        if "safety_stop"          in evts: return "collision"
        if "same_lane_emergency"  in evts: return "collision"
        if "same_lane_slow"       in evts: return "near_miss"
        return "safe"
...
def frames_to_iv_arrays(frames: List[dict]) -> dict:
    ...
    and the new rich-dict format::

        {"1-2": {"euclidean_px": 123.4, "same_lane": True,
                 "lane_a": 1, "lane_b": 1, "seg_delta": 42}}
...
def plot_iv_distance(frames: List[dict], meta: dict, outdir: str) -> None:
    ...
    Two sub-plots:
      Top    – same-lane pairs  (safety-critical: D_SAFE / D_WARN bands)
    ...
    # legacy / unknown: show in same-lane panel by default
    ...
    if same_pairs:
        ...
        ax.fill_between(all_t, d_col,  d_safe,  alpha=0.08, color=PAL["d_safe"])
        ax.fill_between(all_t, d_safe, d_warn,  alpha=0.06, color=PAL["d_warn"])
        ax.axhline(d_safe, color=PAL["d_safe"], lw=1.4, ls="--",
                   label=f"D_SAFE {d_safe} px")
        ax.axhline(d_warn, color=PAL["d_warn"], lw=1.2, ls=":",
                   label=f"D_WARN {d_warn} px")
        # secondary axis: seg_delta (path-distance proxy)
...
def plot_waiting_times(zones: dict, meta: dict, outdir: str) -> None:
    ...
    ax.set_xlabel("Interaction"); ax.set_ylabel("Waiting time τᵢ s")
...
def plot_lane_timeline(arrs_by_car: dict, meta: dict, outdir: str) -> None:
    ...
    fig, ax = _fig()
    for car_id, arrs in arrs_by_car.items():
        ax.step(arrs["t"], arrs["lane"], lw=1.4, where="post", label=f"Car {car_id}")
...
def plot_trajectory(runs_data: List[dict], outdir: str, avg_mode: bool = False) -> None:
    ...
    Draws the track ground-truth curves (inner / middle / outer / lane refs)
    on a dark background, then overlays the car position scatter points from
    one or more runs.  In avg_mode (multi-file averaging) all runs are merged
    so the point cloud density reflects the full N-repetition dataset.

    The ground-truth geometry is taken from the first run that has a
    non-empty 'track_ground_truth' key (written once by save_log).
    """
    ...
    ax.tick_params(colors="#7a7974")
    ax.set_aspect("equal")
    ax.invert_yaxis()   # image-space: y increases downward

    # ── ground-truth track curves ─────────────────────────────────────────────
    gt = next((r.get("track_ground_truth") for r in runs_data
               if r.get("track_ground_truth")), None)

    # Ground-truth: only the two lane centrelines (ideal path per lane)
    GT_STYLE = {
    ...
    if gt:
        for key, (col, lw, lbl) in GT_STYLE.items():
            ...
            if pts:
                ...
                arr_closed = np.vstack([arr, arr[[0]]])   # close the loop
    ...
    # ── car position scatter (one colour per car ID) ──────────────────────────
    ...
    _dfov_lbl = f"  dFOV={_dfov}°" if _dfov else ""
    ax.set_title(
...
def process_averaged_files(paths: List[str], outdir: str) -> dict:
    """
    ...
    Summary statistics are recomputed from the first file's frames after the
    summary dict from each run is collected and averaged numerically.
    """
    N_RESAMPLE = 1000
    ...
    # Base meta from first file; override n_frames with mean
    ...
    for car_id in all_car_ids:
        ...
        def _majority_vote(step_i: int) -> str:
            ...
            for ra in run_arrs:
                ...
                votes.append(ra["safety"][idx_r])
            return Counter(votes).most_common(1)[0][0]

        averaged["safety"] = [_majority_vote(i) for i in range(N_RESAMPLE)]
        avg_arrs_by_car[car_id] = averaged

    # ── output directory for this averaged group ──────────────────────────────
    rundir = _results_dir(meta)

    # ── charts (use averaged arrays) ─────────────────────────────────────────
...
def process_file(path: str, outdir: str) -> dict:
    ...
    summary_by_car = {cid: compute_summary(frames, zones, meta, cid) for cid in car_ids}
    data["summary_by_car"] = summary_by_car
    data["summary"]        = summary_by_car.get(car_ids[0], {}) if car_ids else {}

    # Write updated JSON with summaries
...
def main():
    ...
    # ── split files: averaged group vs. individual runs ───────────────────────
    if args.avg_files and len(args.files) >= args.avg_files:
        ...
        avg_run    = process_averaged_files(avg_group, args.out)
        ind_runs   = [process_file(p, args.out) for p in remaining]
